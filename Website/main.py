    ...
    if request.method == 'POST':
        ...
        for user in existing_users:
            if username == user[1]:
                ...
                return render_template('add_user.html', added_username=None, message=message)
...
def remove_user():
    ...
    if request.method == 'POST':
        ...
        for user in existing_users:
            if username == user[0]:  
                user_found = True
                cursor.execute('DELETE FROM pswd WHERE username=%s', (user[0],))  
                db.commit()
...
def process_entext_option():
    option = request.json.get('option')
    textbox_value = request.json.get('textboxValue') 
...
def fetch_data_from_database(selected_date):
    ...
    try:
        with ms.connect(
            ...
            result_list.append('<body style="background-color: #05162c; text-align: center;">')
            result_list.append('<style>')
            result_list.append('button {')
            result_list.append('    background-color: #104386;')
            result_list.append('    color: #ffffff; /* Set text color to white */')
            result_list.append('    border: none;')
            result_list.append('    border-radius: 40px;')
            result_list.append('    cursor: pointer;')
            result_list.append('    padding: 10px 20px;')
            result_list.append('    font-size: 20px;')
            result_list.append('    font-family: \'Lucida Sans\', \'Lucida Sans Regular\', \'Lucida Grande\', \'Lucida Sans Unicode\', Geneva, Verdana, sans-serif;')
            result_list.append('    max-width: 400px;')
            result_list.append('    margin: 20px; /* Reduced margin */')
            result_list.append('    margin-left: -1300px; /* Reduced margin */')
            result_list.append('}')
            result_list.append('')
            result_list.append('@media only screen and (max-width: 600px) {')
            result_list.append('    button {')
            result_list.append('        font-size: 20px; /* Adjusted font size for smaller screens */')
            result_list.append('    }')
            result_list.append('}')
            result_list.append('button:hover {')
            result_list.append('    background-color: #010a17;')
            result_list.append('}')
            result_list.append('</style>')
            result_list.append('<a href="/rep_home"><button>Go Back</button></a>')
            result_list.append('<img src="/static/ParkingIcon1.png" alt="Logo" style="width: 500px; height: auto; margin: 0 auto; display: block; margin-bottom: 20px; margin-top:-100px;">')
            result_list.append('<table style="width:50%; border-collapse: collapse; border: 2px solid white; color: white; font-family: Arial, sans-serif; font-size: 25px; margin: 0 auto;">')
            result_list.append('<tr style="border: 2px solid white; font-size: 120% ;"><th style="border-right: 2px solid white;">Car Number</th><th>Money Paid</th></tr>')
            for row in data:
                result_list.append('<tr style="border: 2px solid white;"><td style="border-right: 2px solid white;">{}</td><td>{}</td></tr>'.format(*row))
            result_list.append('<tr style="border: 2px solid white;"><td style="border-right: 2px solid white;">Total Money Collected:</td><td>{}</td></tr>'.format(total_money_paid))
            result_list.append('<tr style="border: 2px solid white;"><td style="border-right: 2px solid white;">Total Count of Vehicles:</td><td>{}</td></tr>'.format(total_count))
...
def fetch_data_from_database2():
    ...
    try:
        with ms.connect(
            ...
            result_list.append('<table style="width:30%; border-collapse: collapse; border: 2px solid white; color: white; font-family: Arial, sans-serif; font-size: 20px; margin: 0 auto; background-color: #05162c;">')
            result_list.append('<tr style="border: 2px solid white;"><th>Car Number</th></tr>')

            for row in data:
                if len(row) >= 1:
                    cell_style = 'style="border: 2px solid white; padding: 8px; text-align: center;"'
                    result_list.append('<tr {}><td>{}</td></tr>'.format(cell_style, row[0]))
                else:
                    result_list.append('<tr style="border: 2px solid white;"><td>Invalid Data</td></tr>')

            result_list.append('<tr style="border: 2px solid white;"><td style="border-top: 2px solid white; padding-top: 8px; text-align: center;">Total Count of Vehicle: {}</td></tr>'.format(total_count))
            result_list.append('</table>')
...
def live():
    ...
    logo_html = '<img src="/static/ParkingIcon1.png" alt="Logo" style="width: 500px; height: auto; margin-bottom: 20px;">'

    button_container_html = '<div style="position: absolute; top: 10px; left: 10px;">'
    button_html = '<a href="javascript:history.back()"><button style="background-color: #104386; color: #ffffff; border: none; border-radius: 40px; cursor: pointer; padding: 10px 20px; font-size: 30px; font-family: \'Lucida Sans\', \'Lucida Sans Regular\', \'Lucida Grande\', \'Lucida Sans Unicode\', Geneva, Verdana, sans-serif;" onmouseover="this.style.backgroundColor=\'#010a17\'" onmouseout="this.style.backgroundColor=\'#104386\'">Go Back</button></a>'
    button_container_html_end = '</div>'

    return f'<body style="background-color: #05162c; text-align: center;">{logo_html}{table_html}{button_container_html}{button_html}{button_container_html_end}</body>'

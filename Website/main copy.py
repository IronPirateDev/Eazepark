...
def fetch_data_from_database(selected_date):
    ...
    try:
        with ms.connect(
            ...
            result_list.append('<img src="static\Logo.jpg" alt="Logo" style="width: 500px; height: auto; margin-bottom: 20px;">')
            result_list.append('<table style="width:100%; border-collapse: collapse; border: 2px solid white; color: white; font-family: Arial, sans-serif; font-size: 20px; margin: 0 auto;">')
            for col in cursor.description:
                result_list.append('<th style="padding: 20px; border: 2px solid white; color: white;">{}</th>'.format(col[0]))
            result_list.append('</tr>')
            for row in data:
                result_list.append('<tr>{}</tr>'.format(''.join('<td style="padding: 15px; border: 2px solid white; color: white;">{}</td>'.format(cell) for cell in row)))
